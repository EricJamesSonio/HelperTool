...
async function writeFile(filePath, rules) {
  if (!Array.isArray(rules)) throw new Error('Rules must be an array');
  for (const r of rules) {
    if (typeof r !== 'string') throw new Error(`Invalid rule: ${JSON.stringify(r)}`);
  }
  await fs.promises.writeFile(filePath, JSON.stringify(rules, null, 2), 'utf8');
}

async function _getCached(path, cache) {
  if (cache.data && Date.now() - cache.ts < CACHE_TTL) return cache.data;
  cache.data = await readFile(path);
  cache.ts = Date.now();
  return cache.data;
}
...
function register(shared) {
  ipcMain.handle('docignore:get-global', async () => {
    return await _getCached(getGlobalPath(), _globalCache);
  });

  ipcMain.handle('docignore:set-global', async (event, { rules }) => {
    try {
      await writeFile(getGlobalPath(), rules);
  ...
  ipcMain.handle('docignore:get-repo', async (event, { repoPath }) => {
    ...
    return await _getCached(filePath, cache);
  });
