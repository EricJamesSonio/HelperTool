...
function register() {
  ...
  ipcMain.handle('codebaseMap:generate', async (event, { repoPath }) => {
    try {
      const repo = repositories.getByPath(repoPath);
      if (!repo) return { error: 'Repository not found' };

      const files = indexedFiles.getByRepo(repo.id);
      const allSymbols = symbols.getAllByRepo(repo.id);
      const allImports = imports.getAllByRepo(repo.id);

      const byFileId = {};
      for (const f of files) byFileId[f.id] = f;
      ...
      const textMap = _generateTextMap(repoPath, repo, files, modules, keyFileScores, totalLangs, totalSymbolTypes, circularDeps);
...
function _findCircular(moduleFiles, depsByFile, getModule) {
  const modGraph = {};
  for (const [mod, fpaths] of Object.entries(moduleFiles)) {
...
function _generateTextMap(repoPath, repo, files, modules, keyFileScores, totalLangs, totalSymbolTypes, cycles) {
  const lines = [];
  const repoName = repo.name || repoPath.split(/[\\/]/).pop() || 'Repository';
  ...
  lines.push(`- **${files.length}** files total, **${allSymbolCount(files, modules)}** symbols, **${keyFileScores.reduce((s, k) => s + k.importCount, 0)}** imports`);
...
function allSymbolCount(files, modules) {
