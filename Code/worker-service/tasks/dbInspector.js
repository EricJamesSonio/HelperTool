...
async function connectMongo(conn) {
  ...
  const url = conn.connection_string ||
    `mongodb://${conn.username ? encodeURIComponent(conn.username) + ':' + encodeURIComponent(conn.password) + '@' : ''}${conn.host || 'localhost'}:${conn.port || 27017}/${conn.database || ''}`;
...
async function extractPostgresSchema(client) {
  ...
  for (const row of tableRes.rows) {
    ...
    tables.push({ name: row.table_name, rowCount: row.row_count || 0, schemaName: 'public', columns, indexes });
  }
...
async function extractSqliteSchema(client) {
  ...
  for (const row of tableRows) {
    ...
    const colInfos = client.prepare(`PRAGMA table_info(\`${row.name}\`)`).all();
    ...
    const idxList = client.prepare(`PRAGMA index_list(\`${row.name}\`)`).all();
    ...
    let rowCount = 0;
    try {
      const countRow = client.prepare(`SELECT COUNT(*) as cnt FROM \`${row.name}\``).get();
      rowCount = countRow.cnt;
    } catch (_) {}

    tables.push({ name: row.name, rowCount, schemaName: null, columns, indexes });

    const fkInfos = client.prepare(`PRAGMA foreign_key_list(\`${row.name}\`)`).all();
    for (const fk of fkInfos) {
      relationships.push({
        constraintName: fk.id ? `fk_${row.name}_${fk.id}` : null,
...
async function extractMongoSchema(client) {
  ...
  for (const coll of collections) {
    ...
    try {
      docCount = await db.collection(coll.name).countDocuments({}, { timeout: 3000 });
    ...
    tables.push({ name: coll.name, rowCount: docCount, schemaName: db.databaseName, columns, indexes: [] });
  }
...
async function scan(conn) {
  const client = await connect(conn);
  const schema = await extractSchema(client, conn.type);
  return { success: true, schema };
}

async function executeQuery(conn, query, queryTimeout) {
  const client = await connect(conn);
  const timer = setTimeout(() => { try { client.end(); } catch (_) {} }, queryTimeout || 30000);
  try {
...
module.exports = async function (type, payload) {
  switch (type) {
    case 'db:testConnection':
      return testConnection(payload.conn);
    case 'db:scan':
      return scan(payload.conn);
    case 'db:executeQuery':
      return executeQuery(payload.conn, payload.query, payload.timeout);
    default:
      throw new Error('Unknown dbInspector task type: ' + type);
  }
};
